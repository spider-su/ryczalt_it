...
export function NotificationSettingsScreen({ visible, onClose }: { visible: boolean; onClose: () => void }) {
  ...
  React.useEffect(() => {
    ...
    Promise.all([getNotificationPreferences(profileId), permissionStatus()]).then(([value, permission]) => { if (!active) return; setDraft(value); setPermissionRevoked(value.enabled && permission !== Notifications.PermissionStatus.GRANTED); }).catch(() => active && setError(true)).finally(() => active && setLoaded(true));
    return () => { active = false; };
  }, [visible, profileId]);
  async function save() {
    ...
    try {
      if (draft.enabled && !(await requestPermission())) { setPermissionRevoked(true); setError(true); return; }
